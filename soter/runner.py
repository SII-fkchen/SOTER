"""
Training runner for SOTER.

Part of the training pipeline is adapted from MIRA (https://github.com/microsoft/MIRA).
"""
import os
import math
import random
...
from soter.datasets.mira_dataset import MIRADataset
from soter.datasets.mira_window_dataset import MIRAWindowDataset, TimeAwareWindowDataset
from soter.models.modeling_soter import SoterForPrediction, SoterConfig
from soter.trainer.hf_trainer import SoterTrainingArguments, SoterTrainer
from soter.utils.dist_util import get_world_size
from soter.utils.log_util import logger, log_in_local_rank_0
...
class MIRARunner:
    ...
    def load_model(self, model_path: str = None, from_scatch: bool = False, **kwargs):
        ...
        if from_scatch:
            config = SoterConfig.from_pretrained(model_path, _attn_implementation=attn, apply_aux_loss=False)
            model = SoterForPrediction(config)
        else:
            kwargs['apply_aux_loss'] = False
            model = SoterForPrediction.from_pretrained(model_path, **kwargs)
        # if from_scatch:
        #     config = SoterConfig.from_pretrained(model_path, _attn_implementation=attn)
        #     model = SoterForPrediction(config)
        # else:
        #     model = SoterForPrediction.from_pretrained(model_path, **kwargs)
        return model
    ...
    def train_model(self, from_scratch: bool = False, resume_from_checkpoint: str = None, **kwargs):
        ...
        if os.path.isdir(data_path):
            ...
            if not jsonl_files:
                raise ValueError(f"No .jsonl dataset files found in directory {data_path}.")
        elif data_path.endswith('.jsonl'):
            jsonl_files = [data_path]
        else:
            raise ValueError("data_path must be a .jsonl file, or a directory path containing multiple .jsonl files.")

        # if not data_path.endswith('.jsonl'):
        #     raise ValueError(
        #         "SOTER Architecture Strict Enforcement: "
        #         "The training dataset MUST be in `.jsonl` format to preserve precise physical "
        #         "timestamps and valid loss masks. Please preprocess your data into Time-Aware JSONL."
        #     )
        
        log_in_local_rank_0(f'Detected {len(jsonl_files)} Time-Aware JSONL dataset(s), loading...')
        ...
        for file_path in jsonl_files:
            log_in_local_rank_0(f'Processing dataset: {file_path}')
            ...
            window_dataset = TimeAwareWindowDataset(
                ...
                prediction_length=0, 
                min_valid_history=train_config.get('min_valid_history', 1) 
            )
            all_window_datasets.append(window_dataset)

        # Core: use ConcatDataset to logically concatenate multiple datasets
        train_ds = ConcatDataset(all_window_datasets)
        log_in_local_rank_0(f'All datasets concatenated; total number of available sliding windows: {len(train_ds)}')
        ...
        training_args = SoterTrainingArguments(
        ...
        trainer = SoterTrainer(
            model=model,
            args=training_args,
            train_dataset=train_ds,
            # data_collator=time_aware_collate_fn if isinstance(train_ds, TimeAwareWindowDataset) else None,
            data_collator=time_aware_collate_fn if is_time_aware_dataset else None,
        )
